...
// ─── Constants ────────────────────────────────────────────────────────────────
const DEFAULT_COURSE_NAME = "Java In-Depth - Udemy";
const DEFAULT_LESSON_NAME = "02 - JVM y Tipos de Datos";

function cleanCourseField(value: string): string {
  return value
    .replace(/\s+/g, " ")
    .replace(/^Course:\s*/i, "")
...
function isBadCourseField(value: string): boolean {
  const normalized = cleanCourseField(value).toLowerCase();
  return !normalized
...
export function StudyAgentTab({ session }: { session?: Session }) {
  ...
  const [courseName, setCourseName]   = usePersistedState<string>("agent_course_name", DEFAULT_COURSE_NAME);
  const [lessonName, setLessonName]   = usePersistedState<string>("agent_lesson_name", DEFAULT_LESSON_NAME);
  const [genStep, setGenStep]         = useState(0);

  const [content, setContent]                       = useState<StudyContent | null>(null);
  const [transcript, setTranscript]                 = useState<TranscriptState>({ status: "idle", text: "", source: "none", cueCount: 0, truncated: false });
  ...
  const latestTranscriptCueCountRef = useRef(0);
  ...
  const resetLessonProgress = useCallback(() => {
    ...
    setAnkiExported(false);
    setShowAnkiGuide(false);
    setShowHint(false);
  ...
  const requestStudyTranscript = useCallback(() => {
    setTranscript(prev => prev.status === "ready" ? prev : { ...prev, status: "loading" });
    contentBridge.sendToContent({ type: "GET_TRANSCRIPT", payload: { maxChars: 22000 } });
  }, []);

  const applyCourseContext = useCallback((payload: {
    lectureKey?: unknown;
    courseTitle?: unknown;
    lectureTitle?: unknown;
    cueCount?: unknown;
  } | undefined) => {
    const lectureKey = typeof payload?.lectureKey === "string" ? payload.lectureKey : null;
    const courseTitle = typeof payload?.courseTitle === "string" ? cleanCourseField(payload.courseTitle) : "";
    const lectureTitle = typeof payload?.lectureTitle === "string" ? cleanCourseField(payload.lectureTitle) : "";
    const cueCount = typeof payload?.cueCount === "number" && Number.isFinite(payload.cueCount)
      ? Math.max(0, Math.round(payload.cueCount))
      : 0;
    const lectureChanged = Boolean(lectureKey && lectureKey !== activeLectureKeyRef.current);

    if (lectureChanged) {
      activeLectureKeyRef.current = lectureKey;
      latestTranscriptCueCountRef.current = 0;
      resetLessonProgress();
      setTranscript({ status: "loading", text: "", source: "none", cueCount: 0, truncated: false });
    }

    if (courseTitle && !isBadCourseField(courseTitle) && (courseName === DEFAULT_COURSE_NAME || isBadCourseField(courseName))) {
      setCourseName(courseTitle);
    }

    if (
      lectureTitle
      && !isBadCourseField(lectureTitle)
      && (lectureChanged || lessonName === DEFAULT_LESSON_NAME || isBadCourseField(lessonName))
    ) {
      setLessonName(lectureTitle);
    }

    if (!lectureChanged && cueCount > latestTranscriptCueCountRef.current) {
      latestTranscriptCueCountRef.current = cueCount;
      setTranscript(prev => prev.status === "ready"
        ? { ...prev, cueCount }
        : prev
      );
      ...
      transcriptRefreshTimerRef.current = window.setTimeout(() => {
        ...
        requestStudyTranscript();
      }, 700);
    }

    if (lectureChanged) {
      [250, 1800, 4200, 8000, 14000, 22000].forEach((delay) => {
        setTimeout(requestStudyTranscript, delay);
      });
    }
  }, [courseName, lessonName, requestStudyTranscript, resetLessonProgress, setCourseName, setLessonName]);
  ...
  useEffect(() => {
    const unsubscribe = contentBridge.onMessageFromContent((msg) => {
      ...
      if (msg.type === "LECTURE_CONTEXT_UPDATE" || msg.type === "PONG") {
        applyCourseContext(payload);
        return;
      }
      ...
      if (msg.type !== "TRANSCRIPT_RESULT") return;
    ...
    requestStudyTranscript();
    const retryDelays = [700, 1800, 3500, 6500, 10000, 16000, 24000];
    const retries = retryDelays.map((delay) => setTimeout(requestStudyTranscript, delay));
  ...
  }, [applyCourseContext, requestStudyTranscript]);
  ...
  // Once questions are complete, concepts stay locked as done so back-nav cannot regress the flow.
  ...
  const transcriptPartial = transcript.status === "ready" && transcript.cueCount > 0 && transcript.cueCount < 8;
  const transcriptReady = transcript.status === "ready";
  const transcriptLabel =
    transcript.status === "loading"
      ? "Buscando transcripción"
      : transcriptPartial
        ? "Capturando transcripción"
      : transcriptReady
        ? "Transcripción capturada"
        : "Transcripción no detectada";
  const transcriptCountLabel = transcriptReady
    ? `${transcript.cueCount || 0} líneas${transcriptPartial ? "..." : ""}`
    : transcript.status === "loading"
      ? "..."
      : "0 líneas";
  const transcriptCardClass = transcriptReady && !transcriptPartial
    ? "bg-emerald-500/6 border-emerald-500/12"
    : transcript.status === "loading"
      ? "bg-amber-500/6 border-amber-500/12"
      : transcriptPartial
        ? "bg-amber-500/6 border-amber-500/12"
      : "bg-red-500/6 border-red-500/12";
  const transcriptDotClass = transcriptReady && !transcriptPartial
    ? "bg-emerald-400"
    : transcript.status === "loading"
      ? "bg-amber-400"
      : transcriptPartial
        ? "bg-amber-400"
      : "bg-red-400";
  const transcriptTextClass = transcriptReady && !transcriptPartial
    ? "text-emerald-400"
    : transcript.status === "loading"
      ? "text-amber-400"
      : transcriptPartial
      ...
      <AnimatePresence mode="wait">
        ...
        {phase === "objective" && (
          <motion.div key="obj" initial={{ opacity: 0, y: 10, filter: "blur(2px)" }} animate={{ opacity: 1, y: 0, filter: "blur(0px)" }} exit={{ opacity: 0, y: -10, filter: "blur(2px)" }} transition={{ duration: 0.3 }}
            ...
            <button
              onClick={requestStudyTranscript}
              title={transcriptReady ? `Fuente: ${transcript.source}${transcript.truncated ? " · truncada" : ""}` : "Volver a buscar transcripción"}
              className={`w-full flex items-center gap-2.5 border rounded-lg px-3 py-2 text-left transition-all hover:border-emerald-500/22 ${transcriptCardClass}`}
            >
              <div className={`w-1.5 h-1.5 rounded-full animate-pulse shrink-0 ${transcriptDotClass}`}/>
              <span className="text-white/35 text-[11px]">{transcriptLabel}</span>
              <span className={`${transcriptTextClass} text-[11px] font-mono ml-auto`}>{transcriptCountLabel}</span>
            </button>
